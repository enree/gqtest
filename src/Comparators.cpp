    ...
    bool ignoreWhitespace)
{
    ...
    return ::testing::AssertionFailure()
           << QString("Expected '%1' does not match actual '%2'")
                  .arg(expected, actual)
                  .toStdString();
}
...
public:
    ...
    compare(const QVariant& expected, const QVariant& actual)
        = 0;
...
public:
    ...
    compare(const QVariant& expected, const QVariant& actual) override
    {
...
ComparatorPtr getComparator(int userType)
{
    ...
    {
        comparators.insert(
            QVariant::fromValue(QUuid()).userType(),
            std::make_shared<CustomComparator<QUuid>>());
    }
    ...
    const QObject& expected, const QObject& actual, const char* name)
{
    ...
    if (expectedProperty.type() < QVariant::UserType)
    {
        if (actualProperty != expectedProperty)
        {
            return ::testing::AssertionFailure()
                   << QString(
                          "Property '%1' mismatch. Expected='%2'', actual='%3'")
                          .arg(
                              name,
    ...
    else
    {
        if (actualProperty.userType() == expectedProperty.userType())
        {
            ...
            {
                return ::testing::AssertionFailure()
                       << QString("No comparator for user type '%1'")
        ...
        {
            return ::testing::AssertionFailure()
                   << QString("Property '%1' mismatch. User types are differ")
                          .arg(name)
                          .toStdString();
        }
    }
...
testing::AssertionResult
compareOwnProperties(const QObject& expected, const QObject& actual)
{
...
{
    testing::AssertionResult compareOwn
        = compareOwnProperties(expected, actual);
...
testing::AssertionResult isEqual(int expected, int actual)
{
    if (expected != actual)
    {
        return testing::AssertionFailure() << QString("Expected=%1, actual=%2")
                                                  .arg(expected)
                                                  .arg(actual)
                                                  .toStdString();
    }
